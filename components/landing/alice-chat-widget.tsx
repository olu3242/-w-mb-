...
export default function AliceChatWidget() {
  ...
  return (
    <div ref={widgetRef} className="font-poppins">
      {/* --- Chat Panel — fixed, anchored bottom-right, grows upward --- */}
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            transition={{ duration: 0.4, ease: [0.22, 1, 0.36, 1] as const }}
            className="fixed bottom-4 right-4 md:bottom-8 md:right-8 z-[50] w-[350px] md:w-[400px] h-[500px] md:h-[460px] flex flex-col overflow-hidden bg-zinc-950/90 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] shadow-[0_30px_60px_rgba(0,0,0,0.6)]"
          >
            {/* --- Header --- */}
            <div className="p-6 bg-white/5 border-b border-white/5 flex justify-between items-center">
              <div className="flex items-center gap-4">
      ...
      {/* --- Toggle Button — fixed independently, never moves --- */}
      <AnimatePresence>
        {!open && (
          <motion.div
            ...
            whileHover={{ scale: 1.05, rotate: 5 }}
            whileTap={{ scale: 0.95 }}
            className="fixed bottom-4 right-4 md:bottom-8 md:right-8 z-[50] w-20 h-20 bg-emerald-600 rounded-[2rem] flex items-center justify-center shadow-[0_15px_40px_rgba(16,185,129,0.4)] cursor-pointer"
          >
            <MessageSquare size={30} className="text-white" />
            <span className="absolute top-5 right-5 w-3 h-3 bg-white rounded-full border-2 border-emerald-500" />
          </motion.div>
